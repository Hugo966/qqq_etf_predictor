...
def predecir_modelo_autoregresivo(TS, anio, fs):
    ...
    model = load_model("C:\\Users\\Hugo\\Documents\\Info\\Master\\TFM\\Codigos\\RNNs\\modelos_y_graficas\\Ronda5_vol_sent\\modelo_2(sin_dropout)_240ts_100e_es46_valloss0,0126.h5", custom_objects={'rmse_close_only': rmse_close_only})
    ...
    n_features = X.shape[2] #Número de features, en este caso open, sentiment y volume
    ...
    while cont < n_preds and min(fs, n_preds - (cont)) > 0: 
        for j in range(min(fs, n_preds - (cont))): #Aseguramos que no se salga del rango
            ...
            cont += 1       
            r = tcurrent.reshape(1, TS, X.shape[2]) #r es la ventana de entrada, contiene los TS pasos más recientes, cada uno con sus n_features (open, sentiment y volume en este caso)
    ...
    rmse_volume = np.sqrt(mean_squared_error(y_test[:, 2], auto_preds[:, 2]))
    
    print(f"RMSE 'close': {rmse_close:.4f}")
    print(f"RMSE 'sentiment': {rmse_sent:.4f}")
    print(f"RMSE 'volume': {rmse_volume:.4f}")
    ...
    html_path = (
            f"C:\\Users\\Hugo\\Documents\\Info\\Master\\TFM\\Codigos\\RNNs\\modelos_y_graficas\\Ronda5_vol_sent\\modelo_2(sin_dropout)_240ts_100e_es46_valloss0,0126_{anio}_fs{fs}_rmse{rmse_close:.2f}.html"
    )
    ...
    html_path_sentimiento = (
        f"C:\\Users\\Hugo\\Documents\\Info\\Master\\TFM\\Codigos\\RNNs\\modelos_y_graficas\\Ronda5_vol_sent\\SENTIMENTmodelo_2(sin_dropout)_240ts_100e_es46_valloss0,0126_{anio}_fs{fs}_rmse{rmse_close:.2f}.html"
    )
    ...
    #Gráfico volume
    fig2 = go.Figure()
    #Serie de entrenamiento (real) volume
    fig2.add_trace(
        go.Scatter(x=train_idx, y=y_train[:, 2], mode='lines', name='Train (real)', line=dict(width=1, color='blue'))
    )
    #Serie de test (real) volume
    fig2.add_trace(
        go.Scatter(x=test_idx, y=y_test[:, 2], mode='lines', name='Test (real)', line=dict(width=1, color='orange'))
    )
    #Predicción autoregresiva volume
    fig2.add_trace(
        go.Scatter(x=y_preds, y=auto_preds[:, 2], mode='lines', name='Predicción autoregresiva', line=dict(width=1, color='green'))
    )
    #Diseño
    fig2.update_layout(
        title=f"Volumen real vs Predicción autorregresiva año {anio} y fs={fs}",
        xaxis_title="Tiempo",
        yaxis_title="Volumen",
        legend=dict(yanchor='top', y=0.99, xanchor='left', x=0.01),
        xaxis=dict(
    ...
    #Guardamos la figura en un archivo HTML
    html_path_volume = (
        f"C:\\Users\\Hugo\\Documents\\Info\\Master\\TFM\\Codigos\\RNNs\\modelos_y_graficas\\Ronda5_vol_sent\\VOLUMEmodelo_2(sin_dropout)_240ts_100e_es46_valloss0,0126_{anio}_fs{fs}_rmse{rmse_close:.2f}.html"
    )
    fig2.write_html(html_path_volume)
...
TS = 240
anios = [2021, 2022, 2023, 2024]  # Años a predecir
fss = [3, 6, 12, 24, 48]  
